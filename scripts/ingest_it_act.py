"""Ingest Income-tax Act, 1961 from scraped JSON into Nyaya Ledger corpus.

Usage:
    1. Run scripts/scrape_it_act_console.js in your browser on
       https://www.incometaxindia.gov.in/income-tax-act
    2. Save the downloaded JSON to data/Law/base_laws/income_tax_act_1961.json
    3. Run: python3 scripts/ingest_it_act.py

This script converts the scraped JSON into:
...
The sourceHash values are computed against extracted_text.json's "text" field,
which is the single source of truth for span validation.
"""
...
DATA_FILE = REPO_ROOT / "data" / "Law" / "base_laws" / "income_tax_act_1961.json"
SOURCE_DIR = REPO_ROOT / "sources" / "in" / "union" / "acts" / "income-tax-act-1961"
CORPUS_FILE = (
    REPO_ROOT / "corpus" / "in" / "union" / "acts" / "income-tax-act-1961" / "act.xml"
)

CANONICAL_ID = "/in/union/acts/income-tax-act-1961"
DOCUMENT_TYPE = "act"
TITLE = "The Income-tax Act, 1961"
EFFECTIVE_DATE = "1961-09-01"
PUBLICATION_DATE = "1961-08-24"
ISSUING_AUTHORITY = "/in/authority/unknown"
SOURCE_URL = "https://www.incometaxindia.gov.in/income-tax-act"

KNOWN_ACT_MAP = {
    "income-tax act": CANONICAL_ID,
    "the income-tax act": CANONICAL_ID,
    "income tax act": CANONICAL_ID,
...
def _find_section_blocks(
    source_text: str, sections: list[dict]
) -> list[tuple[int, int, str]]:
    blocks = []
    for sec in sections:
        label = sec.get("section_number", "")
        marker = f"* Section {label}."
        pos = source_text.find(marker)
        if pos < 0:
            continue
        end = pos + len(marker)
        blocks.append((pos, end, label))
    return blocks
...
def render_act_xml(source_text: str, sections: list[dict], source_sha256: str) -> str:
    ...
    xml.append('  <act name="the_income_tax_act_1961">')
    xml.append("    <meta>")
    ...
    xml.append(f'        <property name="parser_version" value="it-act-scraper-v1"/>')
...
def main() -> None:
    if not DATA_FILE.exists():
        ...
        print("Run scripts/scrape_it_act_console.js in your browser first.")
        sys.exit(1)
    ...
    with_text = sum(
        1 for s in sections if s.get("full_text") or s.get("full_text_plain")
    )
